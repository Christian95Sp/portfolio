...
class Canvas {
    ...
    drawSlate() {
        let grad = this.context.createRadialGradient(VIEW_W * 0.5, (VIEW_H - RETURN_H) * 0.5, TABLE_H * 0.75 * 0.125, VIEW_W * 0.5, (VIEW_H - RETURN_H) * 0.5, TABLE_H * 0.75 * 1.5);
        grad.addColorStop(0, 'rgba(255,255,255,0.05)');
        grad.addColorStop(0.25, 'rgba(255,255,255,0.05)');
        grad.addColorStop(1, 'rgba(255,255,255,0.15)');
        this.context.fillStyle = COLORS.felt;
    ...
    drawPoints() {
        let di = 10,
            ...
            yt3 = yc3 - inc,
            yb3 = yc3 + inc;
        let positions = [[xl1, y1], [xc1, y1], [xr1, y1], [xl1, y2], [xc1, y2], [xr1, y2], [xl2, y1], [xc2, y1], [xr2, y1], [xl2, y2], [xc2, y2], [xr2, y2], [x3, yt3], [x3, yc3], [x3, yb3], [x4, yt3], [x4, yc3], [x4, yb3]];
        this.context.fillStyle = COLORS.brown;
        positions.forEach(coords => {
            let x = coords[0],
                y = coords[1];
    ...
    drawBall(ball) {
        ...
        let offsetX = (x - WALL_DI) / TABLE_W * 2 - 1,
            offsetY = (y - WALL_DI) / TABLE_H * 2 - 1;
        let grad = this.context.createRadialGradient(rad * offsetX, rad * offsetY, rad * 0.125, rad * offsetX, rad * offsetY, rad * 1.5);
        if (ball.eight) {
            grad.addColorStop(0, 'rgba(255,255,255,0.15)');
            grad.addColorStop(1, 'rgba(255,255,255,0.05)');
        } else {
...
class Game {
    ...
    handleCollisionActive({
        ...
        pairs.forEach(({
            ...
            if (coll === 'ballpocket' || coll == 'pocketball') {
                let ball = bodyA.label === 'ball' ? bodyA : bodyB;
                let distance = Math.hypot(bodyA.position.y - bodyB.position.y, bodyA.position.x - bodyB.position.x);
    ...
    handleCollisionStart({
        ...
        pairs.forEach((collision, i) => {
            ...
            let speed = collision.collision.axisBody.speed;
            let coll = bodyA.label + bodyB.label;
            if (!this.firstContact && coll === 'ballball') this.firstContact = [bodyA, bodyB];
            if (coll === 'ballball') {
                let vol = Math.min(0.5, speed) + 0.05;
                let rate = Math.random() - 0.5 + 1;
                this.sounds.ball.rate(rate);
...
if (window.location.href.match(/cpgrid/)) {
    ...
    Events.on(render, 'afterRender', () => {
        game.handleTickAfter({
            x: mouse.position.x,
            y: mouse.position.y
        });
    });
    let constraint = MouseConstraint.create(engine, {
        mouse
    });
    Events.on(constraint, 'mousedown', ({
        mouse
    }) => {
        game.handleMousedown();
    });
    Events.on(constraint, 'mouseup', ({
        mouse
    }) => {
        game.handleMouseup();
    });
    Events.on(engine, 'collisionActive', e => {
        game.handleCollisionActive({
            pairs: e.pairs
        });
    });
    Events.on(engine, 'collisionStart', e => {
        game.handleCollisionStart({
            pairs: e.pairs
        });
    });
